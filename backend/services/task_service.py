...
def submit_task(
    ...
    """Student: Submit task with optional notes and attachment. Allows resubmission when status is rejected."""
    # Allow submit when status is 'review' (first time) or 'rejected' (resubmit)
    existing = supabase.table("task_assignments").select("status").eq("assignment_id", assignment_id).execute()
    if not existing.data:
        return {"success": False, "message": "Assignment not found"}
    if existing.data[0].get("status") not in ("assigned", "in_progress", "review", "rejected"):
        return {"success": False, "message": "Task cannot be submitted in current status"}
    ...
    if submission_file_path is not None:
        updates["submission_file_path"] = submission_file_path
    result = supabase.table("task_assignments").update(updates).eq("assignment_id", assignment_id).execute()

    if not result.data:
        return {"success": False, "message": "Assignment not found"}
    assignment = result.data[0]
    # Versioning: record this submission
    if submission_file_url:
        ver_r = supabase.table("task_submission_versions").select("version").eq("assignment_id", assignment_id).order("version", desc=True).limit(1).execute()
        next_ver = (ver_r.data[0]["version"] + 1) if ver_r.data else 1
        try:
            supabase.table("task_submission_versions").insert({
...
def _allowed_extensions() -> set:
    exts = getattr(settings, "UPLOAD_ALLOWED_EXTENSIONS", "pdf,doc,docx,txt,xls,xlsx") or "pdf,doc,docx,txt,xls,xlsx"
    return {e.strip().lower() for e in exts.split(",") if e.strip()}


def _max_upload_bytes() -> int:
    mb = getattr(settings, "UPLOAD_MAX_SIZE_MB", 10) or 10
    return mb * 1024 * 1024


def validate_upload_file(file_size: int, filename: str):
    # Returns (True, None) if valid, else (False, error_message)
    """Returns (True, None) if valid, else (False, error_message)."""
    if file_size > _max_upload_bytes():
        return False, f"File too large. Max {getattr(settings, 'UPLOAD_MAX_SIZE_MB', 10)}MB."
    ext = (filename or "").split(".")[-1].lower() if "." in (filename or "") else ""
    if not ext or ext not in _allowed_extensions():
        return False, f"File type not allowed. Allowed: {', '.join(sorted(_allowed_extensions()))}"
    return True, None


def upload_submission_file(assignment_id: str, filename: str, file_bytes: bytes, content_type: Optional[str] = None) -> Optional[tuple[str, str]]:
    """Upload file to Supabase Storage. Validates size/type. Returns (public_url, storage_path) or None on failure."""
    ok, err = validate_upload_file(len(file_bytes), filename)
    if not ok:
        return None
...
def get_submission_download_url(assignment_id: str, expires_seconds: int = 3600) -> Optional[dict]:
    """Mentor: Get signed URL for student submission file. Returns {url, expires_in} or None."""
    r = supabase.table("task_assignments").select("submission_file_url, submission_file_path").eq("assignment_id", assignment_id).execute()
    if not r.data:
        return None
    row = r.data[0]
    path = row.get("submission_file_path")
    url = row.get("submission_file_url")
    if path:
        try:
            signed = supabase.storage.from_(settings.STORAGE_BUCKET_SUBMISSIONS).create_signed_url(path, expires_seconds)
            if signed.get("path") and signed.get("signedURL"):
                return {"url": signed["signedURL"], "expires_in": expires_seconds}
...
def upload_review_file(assignment_id: str, filename: str, file_bytes: bytes, content_type: Optional[str] = None) -> Optional[str]:
    """Mentor: Upload review file for assignment. Updates review_file_url. Returns URL or None."""
    ok, _ = validate_upload_file(len(file_bytes), filename)
    if not ok:
        return None
    bucket = settings.STORAGE_BUCKET_SUBMISSIONS
    safe_name = (filename or "review").split("/")[-1][:100]
    path = f"review/{assignment_id}/{uuid.uuid4().hex}_{safe_name}"
    opts = {}
    if content_type:
        opts["content_type"] = content_type
    try:
        supabase.storage.from_(bucket).upload(path, file_bytes, opts)
        public = supabase.storage.from_(bucket).get_public_url(path)
        supabase.table("task_assignments").update({"review_file_url": public}).eq("assignment_id", assignment_id).execute()
        return public
    except Exception:
        return None
